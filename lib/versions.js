...
function normalizeVersion(release) {
    ...
    return {
        ...
        normalized_tag: normalizeTag(release.tag_name),
...
function compareVersions(v1, v2) {
    if (semver.gt(v1.normalized_tag, v2.normalized_tag)) {
        return -1;
    }
    if (semver.lt(v1.normalized_tag, v2.normalized_tag)) {
        return 1;
    }
    return 0;
}
...
Versions.prototype.filter = function(opts) {
    ...
    if (opts.normalized_tag) {
        opts.channel = extractChannel(opts.normalized_tag)
        opts.tag = opts.normalized_tag.split('-')[0]
    }
