...
function Nuts(opts) {
    ...
    this.router.get('/download/version/:normalized_tag/:platform?', this.onDownload);
    this.router.get('/download/:normalized_tag/:filename', this.onDownload);
...
Nuts.prototype.onDownload = function(req, res, next) {
    ...
    var normalized_tag = req.params.normalized_tag;
    ...
    if (!normalized_tag && channel === '*') channel = 'stable';
    ...
    // Fallback to stable channel if no version found
    .fail(function(err) {
...
Nuts.prototype.onUpdate = function(req, res, next) {
    ...
    .then(function(versions) {
        ...
        if (!latest || latest.normalized_tag == tag) return res.status(204).send('No updates');

        var gitFilePath = (channel === '*' ? '/../../../' : '/../../../../../');

        res.set('Cache-Control', 'max-age=300');
        res.status(200).send({
            "url": urljoin(fullUrl, gitFilePath, '/download/version/'+latest.normalized_tag+'/'+platform+'?filetype='+filetype),
            "name": latest.tag,
    ...
    .then(function(versions) {
       ...
       .then(function(content) {
            ...
            releases = _.chain(releases)
                ...
                .map(function(entry) {
                    var gitFilePath = (channel === '*' ? '../../../../' : '../../../../../../');
                    entry.filename = urljoin(fullUrl, gitFilePath, '/download/'+latest.normalized_tag+'/'+entry.filename);
...
// Serve releases notes (this has been disabled)
